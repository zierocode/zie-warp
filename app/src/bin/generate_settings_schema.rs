...
mod tests {
    ...
    #[test]
    fn oss_schema_uses_release_flags_without_preview_or_dogfood_flags() {
        let flags = active_flags_for_channel("oss");

        assert!(RELEASE_FLAGS.iter().all(|flag| flags.contains(flag)));
        assert!(
            PREVIEW_FLAGS
                .iter()
